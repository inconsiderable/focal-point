...
func main() {
	...
	// handle new incoming filter counts
	mind.SetFilterCountCallback(func(fb *FilterCountMessage) {
	...
	completer := func(d prompt.Document) []prompt.Suggest {
		s := []prompt.Suggest{
			...
			{Text: "points", Description: "Show immature count points for all public keys"},
	...
	for {
		...
		case "points":
			...
			var total int64
			lastHeight := tipHeader.Height - COUNTERPOINT_MATURITY
		gpkt:
			for i, pubKey := range pubKeys {
...
func sendConsideration(mind *Mind) (ConsiderationID, error) {
	...
	// create and send send it. by default the consideration expires if not rendered within 3 counts from now
