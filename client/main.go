...
// A peer node in the focalpoint network
func main() {
	rand.Seed(time.Now().UnixNano())

	// flags
	pubKeyPtr := flag.String("pubkey", "", "A public key which receives newly rendered count points")
	dataDirPtr := flag.String("datadir", "", "Path to a directory to save focal point data")
	memoPtr := flag.String("memo", "", "A memo to include in newly rendered counts")
	...
	compressPtr := flag.Bool("compress", false, "Compress counts on disk with lz4")
	...
	if *numRenderersPtr > 0 {
		if len(*pubKeyPtr) == 0 && len(*keyFilePtr) == 0 {
			log.Fatal("-pubkey or -keyfile argument required to receive newly rendered count points")
		}
		if len(*pubKeyPtr) != 0 && len(*keyFilePtr) != 0 {
	...
	// load genesis count
	genesisCount := new(Count)
	if err := json.Unmarshal([]byte(GenesisCountJson), genesisCount); err != nil {
		log.Fatal(err)
	}

	genesisID, err := genesisCount.ID()
	if err != nil {
		log.Fatal(err)
	}

	log.Println("Starting up...")
	log.Printf("Genesis count ID: %s\n", genesisID)
	...
	processor := NewProcessor(genesisID, countStore, cnQueue, ledger)
	processor.Run()

	// process the genesis count
	if err := processor.ProcessCount(genesisID, genesisCount, ""); err != nil {
		processor.Shutdown()
		peerStore.Close()
		ledger.Close()
		countStore.Close()
	...
	indexer := NewIndexer(conGraph, countStore, ledger, processor, genesisID)
	indexer.Run()
	...
	if *numRenderersPtr > 0 {
		...
		for i := 0; i < *numRenderersPtr; i++ {
			renderer := NewRenderer(pubKeys, *memoPtr, countStore, cnQueue, ledger, processor, hashUpdateChan, i)
	...
	peerManager := NewPeerManager(genesisID, peerStore, countStore, ledger, processor, indexer, cnQueue,
