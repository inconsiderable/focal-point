...
// AddBatch adds a batch of considerations to the queue (a count has been disconnected.)
...
func (t *ConsiderationQueueMemory) AddBatch(ids []ConsiderationID, cns []*Consideration, height int64) error {
	...
	// priority for getting into the next count.
	...
	// if we're disconnecting a count we're going to be connecting some shortly.
	return nil
}

// RemoveBatch removes a batch of considerations from the queue (a count has been connected.)
...
func (t *ConsiderationQueueMemory) RemoveBatch(ids []ConsiderationID, height int64, more bool) error {
	...
	if more {
		...
		// until we're done connecting all of the counts we intend to
		return nil
	}
...
func (t *ConsiderationQueueMemory) reprocessQueue(height int64) error {
	...
	for e := tmpQueue.Front(); e != nil; e = e.Next() {
		...
		if !checkConsiderationSeries(cn, height+1) ||
			// check maturity and expiration if included in the next count
