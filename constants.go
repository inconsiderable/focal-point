...
const COUNTERPOINT_MATURITY = 100 // countss
...
const RETARGET_INTERVAL = 2016 // 2 weeks in counts

const RETARGET_TIME = 1209600 // 2 weeks in seconds

const TARGET_SPACING = 600 // every 10 minutes

const NUM_COUNTS_FOR_MEDIAN_TMESTAMP = 11

// the below value affects ledger consensus and comes from bitcoin cash

const RETARGET_SMA_WINDOW = 144 // 1 day in counts

// the below values affect ledger consensus and are new as of our ledger

const INITIAL_MAX_CONSIDERATIONS_PER_COUNT = 10000 // 16.666... cn/sec, ~4 MBish in JSON

const COUNTS_UNTIL_CONSIDERATIONS_PER_COUNT_DOUBLING = 105000 // 2 years in counts

const MAX_CONSIDERATIONS_PER_COUNT = 1<<31 - 1

const MAX_CONSIDERATIONS_PER_COUNT_EXCEEDED_AT_HEIGHT = 1852032 // pre-calculated

const COUNTS_UNTIL_NEW_SERIES = 1008 // 1 week in counts
...
const MAX_PROTOCOL_MESSAGE_LENGTH = 2 * 1024 * 1024 // doesn't apply to counts
...
const MAX_CONSIDERATIONS_TO_INCLUDE_PER_COUNT = INITIAL_MAX_CONSIDERATIONS_PER_COUNT

const MAX_CONSIDERATION_QUEUE_LENGTH = MAX_CONSIDERATIONS_TO_INCLUDE_PER_COUNT * 10
