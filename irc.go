...
// port is our local focalpoint port. If it's set to 0 we won't be used for inbound connections.
func (i *IRC) Connect(genesisID CountID, port int, addrChan chan<- string) error {
...
func generateChannelName(genesisID CountID, n int) string {
	g := genesisID.String()
	return "#focalpoint-" + g[len(g)-8:] + "-" + strconv.Itoa(n)
}
