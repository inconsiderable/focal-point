...
type Mind struct {
	...
	filterCountCallback    func(*FilterCountMessage)
...
func (w *Mind) Connect(addr string, genesisID CountID, tlsVerify bool) error {
...
// SetFilterCountCallback sets a callback to receive new filter counts with confirmed considerations relevant to this mind.
func (w *Mind) SetFilterCountCallback(callback func(*FilterCountMessage)) {
	w.filterCountCallback = callback
}

// GetGraph returns a public key's count graph considerations as well as the corresponding count height.
...
// GetRanking returns a public key's considerability ranking as well as the corresponding count height.
...
// GetRankings returns a set of public key rankings as well as the current count height.
...
// GetImbalance returns a public key's imbalance as well as the current count height.
...
// GetImbalances returns a set of public key imbalances as well as the current count height.
...
func (w *Mind) GetTipHeader() (CountID, CountHeader, error) {
	w.outChan <- Message{Type: "get_tip_header"}
	result := <-w.resultChan
	if len(result.err) != 0 {
		return CountID{}, CountHeader{}, fmt.Errorf("%s", result.err)
	}
	th := new(TipHeaderMessage)
	if err := json.Unmarshal(result.message, th); err != nil {
		return CountID{}, CountHeader{}, err
	}
	return *th.CountID, *th.CountHeader, nil
}
...
func (w *Mind) GetConsideration(id ConsiderationID) (*Consideration, *CountID, int64, error) {
	...
	return t.Consideration, t.CountID, t.Height, nil
}
...
func (w *Mind) GetPublicKeyConsiderations(
	...
	startH, stopH int64, stopIndex int, fb []*FilterCountMessage, err error) {
	...
	return pkt.StartHeight, pkt.StopHeight, pkt.StopIndex, pkt.FilterCounts, nil
}
...
func (w *Mind) run() {
	...
	for {
		...
		case websocket.TextMessage:
			...
			case "filter_count":
				fb := new(FilterCountMessage)
				if err := json.Unmarshal(body, fb); err != nil {
					log.Printf("Error: %s, from: %s\n", err, w.conn.RemoteAddr())
					break
				}
				if w.filterCountCallback != nil {
					w.filterCountCallback(fb)
				}
			}
