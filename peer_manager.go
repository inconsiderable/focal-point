...
func NewPeerManager(
	genesisID CountID, peerStore PeerStorage, countStore CountStorage,
...
func (p *PeerManager) run() {
	...
	for {
		...
		case <-ticker.C:
			...
			if p.dnsseed && p.outboundPeerCount() >= MAX_OUTBOUND_PEER_CONNECTIONS && rand.Intn(2) == 1 {
...
func (p *PeerManager) connectToPeers(ctx context.Context) error {
	...
	if ibd {
		...
		// render as soon as we think we're done with them and find more peers
		want = 1
	} else {
...
func (p *PeerManager) connect(ctx context.Context, addr string) (int, *Peer, error) {
	peer := NewPeer(nil, p.genesisID, p.peerStore, p.countStore, p.ledger, p.processor, p.indexer, p.cnQueue, p.countQueue, p.addrChan)
...
func (p *PeerManager) acceptConnections() {
	...
	peerHandler := func(w http.ResponseWriter, r *http.Request) {
		...
		theirNonce := r.Header.Get("Countpoint-Peer-Nonce")
		...
		theirAddress := r.Header.Get("Countpoint-Peer-Address")
		...
		peer := NewPeer(conn, p.genesisID, p.peerStore, p.countStore, p.ledger, p.processor, p.indexer, p.cnQueue, p.countQueue, p.addrChan)
	...
	// listen for websocket requests using the genesis count ID as the handler pattern
...
// IsInitialCountDownload returns true if it appears we're still syncing the focal point.
func IsInitialCountDownload(ledger Ledger, countStore CountStorage) (bool, int64, error) {
	tipID, tipHeader, _, err := getPointTipHeader(ledger, countStore)
	if err != nil {
		return false, 0, err
	}
	if tipID == nil {
