...
// InvCountMessage is used to communicate counts available for download.
// Type: "inv_count".
type InvCountMessage struct {
	CountIDs []CountID `json:"count_ids"`
}

// GetCountMessage is used to request a count for download.
// Type: "get_count".
type GetCountMessage struct {
	CountID CountID `json:"count_id"`
}

// GetCountByHeightMessage is used to request a count for download.
// Type: "get_count_by_height".
type GetCountByHeightMessage struct {
	Height int64 `json:"height"`
}

// CountMessage is used to send a peer a complete count.
// Type: "count".
type CountMessage struct {
	CountID *CountID `json:"count_id,omitempty"`
	Count   *Count   `json:"count,omitempty"`
}

// GetCountHeaderMessage is used to request a count header.
// Type: "get_count_header".
type GetCountHeaderMessage struct {
	CountID CountID `json:"count_id"`
}

// GetCountHeaderByHeightMessage is used to request a count header.
// Type: "get_count_header_by_height".
type GetCountHeaderByHeightMessage struct {
	Height int64 `json:"height"`
}

// CountHeaderMessage is used to send a peer a count's header.
// Type: "count_header".
type CountHeaderMessage struct {
	CountID     *CountID     `json:"count_id,omitempty"`
	CountHeader *CountHeader `json:"header,omitempty"`
}
...
// TipHeaderMessage is used to send a peer the header for the tip count in the focal point.
...
// FilterCountMessage represents a pared down count containing only considerations relevant to the peer given their filter.
// Type: "filter_count".
type FilterCountMessage struct {
	CountID         CountID           `json:"count_id"`
	Header         *CountHeader      `json:"header"`
	Considerations []*Consideration `json:"considerations"`
}

// FilterConsiderationQueueMessage returns a pared down account of the unconfirmed consideration queue containing only
...
// PublicKeyConsiderationsMessage is used to return a list of count headers and the considerations relevant to
...
type PublicKeyConsiderationsMessage struct {
	...
	FilterCounts []*FilterCountMessage `json:"filter_counts"`
	Error        string               `json:"error,omitempty"`
}
