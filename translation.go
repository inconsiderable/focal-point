...
type Consideration struct {//TODO: Beware of reordering struct fields, IDs seem to be sensitive to field order.
	...
	Matures   int64             `json:"matures,omitempty"` // count height. if set consideration can't be rendered before
	Expires   int64             `json:"expires,omitempty"` // count height. if set consideration can't be rendered after
...
// IsCounterPoint returns true if the consideration is a counterpoint. A counterpoint is the first consideration in every count
// used to recognise the renderer for rendering the count.
func (cn Consideration) IsCounterPoint() bool {
	return cn.By == nil
}
...
func computeConsiderationSeries(isCountpoint bool, height int64) int64 {
	if isCountpoint {
		// countpoints start using the new series right on time
		return height/COUNTS_UNTIL_NEW_SERIES + 1
	}

	// otherwise don't start using a new series until 100 counts in to mitigate
	// potential reorg issues right around the switchover
	return (height-100)/COUNTS_UNTIL_NEW_SERIES + 1
}
